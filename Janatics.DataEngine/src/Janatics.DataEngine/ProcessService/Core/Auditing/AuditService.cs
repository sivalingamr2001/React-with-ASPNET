...
namespace Janatics.DataEngine.Core.Auditing
{
    ...
    public class AuditService : IAuditService
    {
        ...
        private static readonly Regex TypeNameRegex = new(@"^[a-zA-Z_][a-zA-Z0-9_]*$", RegexOptions.Compiled);
        ...
        private async Task<List<FieldMetadata>> LoadFieldMetadataAsync(string entityName, NpgsqlConnection connection)
        {
            ...
            while (await reader.ReadAsync().ConfigureAwait(false))
            {
                var meta = new FieldMetadata
                {
                    ...
                    GroupOptionKey = reader.IsDBNull(6) ? null : reader.GetInt64(6),
            ...
            object? rawValue)
        {
            ...
            try
            {
                var safeLookupTable = SanitizeQualifiedIdentifier(lookupTable);
                var safeIdColumn = SanitizeIdentifier(idColumn);
                var safeLabelColumn = SanitizeIdentifier(labelColumn);
                var safeLabelIdType = SanitizeTypeName(lableiddatatype);

                var sql = $@"
                    SELECT {safeLabelColumn}
                    FROM {safeLookupTable}
                    WHERE {safeIdColumn} = @id :: {safeLabelIdType}
                    LIMIT 1";
            ...
            string idColumnName = "id")
        {
            try
            {
                ...
                var sql = $"SELECT * FROM {SanitizeTableName(tableName)} WHERE {SanitizeIdentifier(idColumnName)} = @id LIMIT 1";
                ...
                if (transaction != null)
                {
                    ...
                    command.Parameters.Add(new NpgsqlParameter("@id", DbType.Guid) { Value = transactionId });
        ...
        {
            if (!TypeNameRegex.IsMatch(typeName))
                throw new ArgumentException($"Invalid type name: {typeName}");
            return typeName;
        }

        private static string SanitizeQualifiedIdentifier(string identifier)
        {
            ...
            {
                if (!IdentifierRegex.IsMatch(segment))
                    throw new ArgumentException($"Invalid qualified identifier: {identifier}");
            }

...
public class AuditDiffBuilder
{
    ...
    private static readonly AsyncLocal<IDataProvider?> _asyncLocalDataProvider = new();
    ...
    private async Task<string> GetDisplayRecordNameAsync(AuditJob job)
    {
            ...
            !string.IsNullOrWhiteSpace(job.TableMetadata?.TableName) && CurrentProvider != null)
        {
            try
            {
                var safeDisplayField = SanitizeIdentifier(displayField);
                var safeIdField = SanitizeIdentifier(job.TableMetadata.IdField ?? "id");
                var safeTableName = SanitizeQualifiedIdentifier(job.TableMetadata.TableName!);

                var query = $"SELECT {safeDisplayField} FROM {safeTableName} WHERE {safeIdField} = @id LIMIT 1";
                var parameters = new Dictionary<string, object> { { "id", job.TransactionId } };

                var result = await CurrentProvider.ExecuteQueryAsync(query, parameters).ConfigureAwait(false);

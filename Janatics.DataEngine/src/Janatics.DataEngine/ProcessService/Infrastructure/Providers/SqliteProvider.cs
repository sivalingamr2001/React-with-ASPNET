...
/// <summary>
/// Enhanced SQLite provider that implements IDataProvider with optimized connection management,
...
public class SqliteProvider : IDataProvider
{
    ...
    public async Task<IDbTransaction> BeginTransactionAsync(IDbConnection connection)
    {
        ...
        _logger.LogDebug("SQLite transaction started");
    ...
    {
        using var command = new SqliteCommand(sql, GetSqliteConnection(transaction), GetSqliteTransaction(transaction));
    ...
    public async Task<DataTable> ExecuteQueryAsync(string sql, Dictionary<string, object> parameters, IDbTransaction? transaction = null)
    {
        ...
        using var command = new SqliteCommand(sql, (SqliteConnection)connection, (SqliteTransaction?)transaction);
        AddParameters(command, parameters);
    ...
    public async Task<object?> ExecuteInsertWithReturnAsync(string tableName, Dictionary<string, object> parameters, string returnColumn, IDbTransaction transaction)
    {
        ...
        var sql =
            ...
            $"VALUES ({string.Join(", ", processedPlaceholders)}); SELECT last_insert_rowid();";
    ...
    public async Task<int> ExecuteUpdateAsync(string tableName, Dictionary<string, object> parameters, Dictionary<string, object> whereConditions, IDbTransaction transaction)
    {
        ...
        foreach (var param in parameters)
        {
            ...
            {
                // SQLite doesn't support NEXTVAL, but we keep the pattern for compatibility
                // In SQLite, you would typically use AUTOINCREMENT or sqlite_sequence
    ...
    public async Task<int> BulkInsertAsync(string tableName, DataTable dataTable, IDbTransaction transaction)
    {
        ...
        _logger.LogInformation("Starting SQLite bulk insert for table {TableName} with {RowCount} rows", tableName, dataTable.Rows.Count);
        
        var connection = GetSqliteConnection(transaction);
        using var command = new SqliteCommand(insertSql, connection, GetSqliteTransaction(transaction));
        
        // Create parameters once
        var parameters = new List<SqliteParameter>();
        foreach (DataColumn column in dataTable.Columns)
        {
            var param = new SqliteParameter(GetParameterPlaceholder(column.ColumnName), null);
        ...
        _logger.LogInformation("SQLite bulk insert completed for table {TableName}, {RowCount} rows inserted", tableName, rowCount);
    ...
    {
        // Split by dot: schema.table (SQLite supports schema but usually main.table or just table)
    ...
    /// <summary>
    /// SQLite does not support connection pooling like PostgreSQL, so this is a no-op
    /// </summary>
    private async Task TerminateOtherIdleConnectionsAsync(IDbConnection connection)
    {
        // SQLite is file-based and does not have a connection pool or backend processes to terminate
        // This method exists for interface parity with PostgreSqlProvider
    ...
    public async Task<int> BulkInsertAsync(string tableName, DataTable dataTable, IDbConnection connection, IDbTransaction? transaction = null)
    {
        ...
        _logger.LogInformation("Starting SQLite bulk insert with shared connection for table {TableName} with {RowCount} rows", tableName, dataTable.Rows.Count);
        ...
        _logger.LogInformation("SQLite bulk insert completed with shared connection for table {TableName}, {RowCount} rows inserted", tableName, rowCount);
    ...
    private (List<string> columns, List<string> placeholders, Dictionary<string, object> parameters) ProcessParametersForSequences(Dictionary<string, object> originalParameters)
    {
        ...
        foreach (var param in originalParameters)
        {
            ...
            if (param.Value is string stringValue)
            {
                ...
                {
                    // SQLite doesn't have native sequences, but we keep the pattern
                    // In practice, use AUTOINCREMENT or ROWID
    ...
    private static SqliteConnection GetSqliteConnection(IDbTransaction transaction)
    {
        ArgumentNullException.ThrowIfNull(transaction);

        return transaction.Connection as SqliteConnection
            ?? throw new InvalidOperationException("The SQLite provider requires an active SqliteConnection on the supplied transaction.");
    }

    private static SqliteTransaction GetSqliteTransaction(IDbTransaction transaction)
    {
        ArgumentNullException.ThrowIfNull(transaction);

        return transaction as SqliteTransaction
            ?? throw new InvalidOperationException("The SQLite provider requires an SqliteTransaction instance.");
    }

...
public class PostgreSqlProvider : IDataProvider
{
    ...
    {
        using var command = new NpgsqlCommand(sql, GetNpgsqlConnection(transaction), GetNpgsqlTransaction(transaction));
    ...
    public async Task<int> BulkInsertAsync(string tableName, DataTable dataTable, IDbTransaction transaction)
    {
        ...
        using var writer = GetNpgsqlConnection(transaction).BeginBinaryImport(copyCommand);
    ...
    private static NpgsqlConnection GetNpgsqlConnection(IDbTransaction transaction)
    {
        ArgumentNullException.ThrowIfNull(transaction);

        return transaction.Connection as NpgsqlConnection
            ?? throw new InvalidOperationException("The PostgreSQL provider requires an active NpgsqlConnection on the supplied transaction.");
    }

    private static NpgsqlTransaction GetNpgsqlTransaction(IDbTransaction transaction)
    {
        ArgumentNullException.ThrowIfNull(transaction);

        return transaction as NpgsqlTransaction
            ?? throw new InvalidOperationException("The PostgreSQL provider requires an NpgsqlTransaction instance.");
    }
